...
| `gate_margin` | `0.0` | candidate must beat the best by strictly more than this margin |
| `validation_depth` | `self-contained` | `map-only` \| `self-contained` \| `verifiers-env` \| `full-ablation` |


**One-line summaries:** gate_margin = how much the candidate must beat the best by. early_stop_patience = consecutive rejections before the loop halts (returns the iteration number).
