...
- Currency: output the lowercase ISO 4217 code — "$" → "usd", "€" → "eur", "£" → "gbp" (pass codes through, e.g. "GBP" → "gbp"). Put only the numeric value in `amount` (no symbol).
- Note: lowercase the memo/note/ref text. If the snippet has no genuine memo, note, or ref (e.g. "No memo"), output null.
